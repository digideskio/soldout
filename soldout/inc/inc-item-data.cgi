...
# 簡単な説明が最後にありますので、ご覧下さい。

@@DEFINE							#★基本データの定義宣言
	version	2002-12-02				#★商品データバージョン表記
	set		HERO_NAME	$$HERO_NAME	#★変数の定義（$HERO_NAMEを定義し、参照可）★2001-06-06追加
									#★$$nameで_config.cgi内の変数$nameを参照

	scale	個			#★デフォルトの数え単位の定義
	type0	全			#★アイテム分類定義(type0は全アイテムの集合体)
	type1	原料		#★type1分類の定義
	type2	本			#type2分類の定義
	type3	薬			#type3分類の定義
	type4	剣			#type4分類の定義
	...
	type6	盾			#type6分類の定義
	type7	杖			#type7分類の定義
	type8	アクセサリ	#type8分類の定義
	type9	地図		#type9分類の定義
	type10	道具		#type10分類の定義
						#↑の様に拡張可
	...
	TimeShopping		10m		#★仕入時間★2001-06-06廃止
	TimeSendItem		30m		#★アイテム仕入/送信時間(基本)★2001-06-06追加
	TimeSendItemPlus	20s		#★アイテム仕入/送信時間(1個辺りの追加時間)★2001-06-06追加★2001-07-29廃止
	TimeSendMoney		20m		#★資金送信時間(基本)★2001-06-06追加
	TimeSendMoneyPlus	50000	#★入金所要時間計算用金額(この金額につきTimeSendMoney時間を消費)★2001-06-06追加
	
	CostShowcase1		0		#★陳列棚1個時維持費
	CostShowcase2		2000	#陳列棚2個時維持費
	CostShowcase3		5000	#陳列棚3個時維持費
	...
	ItemUseTimeRate		1		#★アイテム使用時時間計算補正倍率(@USE内time,exptimeに有効)★2001-06-06追加
	
	#★マクロ定義
	#  この記述例では、変数 TOWN_TYPE (_config.cgiの$TOWN_TYPE) を判断して時間消費パターンを変更します。
	#  sotype1  -> [加工タイプ] 加工時間短縮パターン     (MUTOYSのトパーズ街)
	#  sotype2  -> [資源タイプ] 素材採取時間短縮パターン (MUTOYSのトパーズ郊外)
	#  timehalf -> [先進タイプ] 全時間短縮パターン       (MUTOYSのアメジスト街)
	#  その他   -> [途上タイプ] 通常時間パターン         (MUTOYSのガーネット街)
	#
	#ここで設定されたマクロは、「@@IF towntype eq material @@USEMACRO_timechange」の様な使い方をします。
	#すると、towntype が material の場合のみ、マクロ _timechange が実行され、以降の時間が短縮されます。
	#マクロ _timenormal で時間が元に戻ります。(timehalf以外)
	#
	#この仕組みを理解するにはプログラムの知識が必要になると思います。
	#上記説明で使用方法が分からない場合は、手を付けない方が無難です。
	#
	
	set	TOWN_TYPE	$$TOWN_TYPE	#★_config.cgiで指定した街タイプを取得/設定
	@@IF TOWN_TYPE eq timeharf set TOWN_TYPE timehalf # ver.2002-01-01-a までとの互換確保
	
	set towntype normal                           # 変数 towntype を normal で初期化する
	@@IF TOWN_TYPE eq sotype1 set towntype manufac  # TOWN_TYPE が sotype1 なら変数 towntype を manufac にする
	@@IF TOWN_TYPE eq sotype2 set towntype material # TOWN_TYPE が sotype2 なら変数 towntype を material にする
	#マクロ定義
	@@SETMACRO _timenormal "@@DEFINE ItemUseTimeRate 1"
	@@IF TOWN_TYPE eq timehalf @@SETMACRO _timenormal "@@DEFINE ItemUseTimeRate 0.5" # TOWN_TYPE が timehalf の場合のマクロ定義
	@@IF TOWN_TYPE eq timequarter @@SETMACRO _timenormal "@@DEFINE ItemUseTimeRate 0.25" # TOWN_TYPE が timequarter の場合のマクロ定義
	@@SETMACRO _timechange "@@DEFINE ItemUseTimeRate 0.75"
	#マクロ _timenormal 実行
	@@USEMACRO _timenormal

#★以下の様な書式で街タイプを判定し、マクロを実行できたりします。
@@IF towntype eq material @@USEMACRO_timechange #-------------------------------------------------

@@ITEM				#★アイテムデータ定義宣言
	no		10				#★アイテム番号(1~)
	type	道具			#★アイテム分類(@@DEFINEのtype?の定義を使用)
	code	book-make		#★コード(アイテム毎ユニーク)
	name	本執筆キット	#★名称
	info	本を執筆する為の基本キット	#★説明
	price	5000				#★標準価格
	cost	5000
	limit	10/5
	pop		1d				#★売れ行き率基準(1dに1個売れる確率)
	scale	セット			#★数え単位
	plus	2h				#★市場入荷率(2hに1個入荷する確率)
	@@USE			#★アイテム使用データ定義宣言
		time	6h			#★消費時間
		exp		4%
		exptime	2h
		...
		action	書く		#★行動表示用
		name	『薬調合入門』を執筆する					#★名称
		info	薬の作り方を勉強し、本にしようと思います	#★説明
		okmsg	『薬調合入門』を執筆しました				#★使用成功時メッセージ
			use		1	本執筆キット	#★必要アイテム
			use		30	薬草			#  個数,アイテム名称,消費確率,消費時メッセージの順で引数記述。
			use		10	ポーション		#  消費確率省略時は100%。
			use		10	エーテル		#  メッセージ省略はメッセージ無し。
			get		1	薬調合入門		#★取得アイテム。引数はuseと同じ。
	@@USE
		time	8h
		exp		5%
		exptime	3h
	...
	@@USE
		...
		okmsg	『剣の鍛え方』を執筆しました
			needexp	20%
			need	1	鍛冶屋の技術	#★必要アイテム。useの消費確率0%固定版。
			use		2	本執筆キット
			use		20	鉄塊
			use		1	木刀
			get		1	剣の鍛え方
...
@@ITEM
	...
	info	薬を調合するための技術書です
	price	15000
	limit	10/1	#★所持最大数（書式パターン2）/の右側は市場入荷最大数(1店舗あたり)
					#  10店舗のゲームだと、この例では市場には最大20個入荷する。
	...
	@@use
		time	1h
		exp		1%		#★使用1回で得られる経験ポイント。（経験ポイントはアイテム毎）
		exptime	40m		#★経験ポイント100%の場合の消費時間
		...
		ngmsg	作成に失敗しました…	#★取得アイテムがなかった場合のメッセージ
		func	lostbook				#★独自関数呼び出し(inc-item-function.cgiで定義)
		param	50
			use		20	薬草
			get		20	ポーション	80%
	@@use
...
@@ITEM
	...
	@@use
		...
		scale	セット
		action	作り上げる
		name	$HERO_NAMEの首飾りを作成する		#★$HERO_NAMEを参照
		info	$HERO_NAMEの首飾りを作り上げます
		okmsg	伝説が今、目の前に！
		ngmsg	作成に失敗しました…
		func	lostbook
		param	200
...
@@ITEM
	...
	pop		10m
	plus	300m
	base	500/1500	#★市場での価格変動用。書式は基準個数/半額個数。
						#  在庫が基準個数時は標準価格で、半額個数時に半額になる。
						#  その比率で、基準個数以下の場合はプレミアが付く。
	scale	個
	cost	10			#★1個あたりの維持費額。未設定時は1/10が自動設定される。
	point	10%
@@ITEM
	no		31
	type	薬
...
@@ITEM
	...
	info	お土産に最適
	price	400
	limit	1000			#★所持最大数（書式パターン1）
	base	100/200
	plus	-1h
	pop		1h
	scale	本
	point	90%		#★売却時の人気上昇率補正。100%で±0。
					#  未設定時は100%。
@@ITEM
	no		41
	type	剣
...
@@ITEM
	...
	flag	nobuy
	funct	_local_
		#ショウウィンドウに標準価格未満で陳列中の場合1日10%の無条件人気アップ
		my($ITEM,@DT)=@_;
...
@@ITEM
	...
	@@use
		...
		ngmsg	なにも仕入れられませんでした…
			...
			get		10	鉄の剣				35%
			get		10	鋼鉄の剣			15%
			get		10	ミスリルの剣		09%		ミスリルの剣が安かったです		#★取得確率,メッセージ付きの書式パターン
			get		10	オリハルコンの剣	03%		オリハルコンの剣が掘り出し物として出てました!
		funcb	_local_
			# 1/10の確率で収穫量が2倍になる
...
@@ITEM
	...
	plus	1d
	scale	キット
	flag	noshowcase|nosend|notradein|notradeout		#★商品に特殊属性を付けます
												#  以下の属性キーワードを記述してください。
												#  | で区切ることで複数の属性を付けることが可能です
												#  noshowcase : 陳列販売不可
												#  nosend     : 郵便送付不可
												#  notrash    : 破棄不可
												#  notradein  : 輸入不可
...
@@if shop_icon_header eq "" @@skip
	...
	@@use
		...
		func	_local_
			...
			return '店舗を'.main::GetTagImgShopIcon($DT->{icon}).$USE->{arg}{select_hash}{$DT->{icon}}.'にしました';
		_local_
	@@use
		time	12h
		name	店舗外観変更(中規模)
		...
		argselect	店舗外観選択;101;タイプ1;102;タイプ2;103;タイプ3
			use	1	陳列棚増築取壊キット
		funci	_local_i9
		func	_local_9
	@@use
		...
		price	10000000
		arg		select|nocount
		argselect	店舗外観選択;201;タイプ1;202;タイプ2;203;タイプ3
			use	1	陳列棚増築取壊キット
		funci	_local_i9
		func	_local_9
...
@@ITEM
	...
	@@use
		...
		arg		target|nocount|message40	#★使用時の選択肢指定。|で区切って複数指定可。
								#  nocount -> 使用回数選択なし
								#  target  -> 対象店舗選択
								#  message[1~?] -> メッセージ入力(80byte以下)あり(数値を付けると入力制限文字数を指定できます)
		argmessage	成功時の犯行声明があればどうぞ	#★argにmessageを設定した時の入力ボックスに
													#説明を入れる場合はこのargmessageで設定できます。
													#入力されたメッセージは$USE->{arg}->{message}で取得可。
													#(SJISに変換し、80or指定byte以下に切り詰め、HTMLタグ無効化(EscapeHTML)済み)
			use		1	禁断の書
		func	_local_
			# ★悪い噂を流す作戦
			my $ret;
			
			# 自店とターゲットの順位関係で成功率を変動させる
			...
			if(rand(1000)<$proba)
			{
				...
				WriteLog(2,0,$DT->{shopname}.'からの犯行声明:「'.$USE->{arg}{message}.'」') if $USE->{arg}{message};
			}
			else
			{
				...
				WriteLog(0,$DT->{id},$ret);
				WriteLog(3,0,$DT->{shopname}."が".$DTS->{shopname}.'の悪い噂を流そうと画策していたようです');
			}
			return $ret;
			_local_
	@@use
			...
			if(rand(1000)<$proba)
			{
				if($DTS->{item}[@@ITEMNO"店番ロボット"-1])
				{
					...
					if(rand(1000)<700)
					{
						...
						WriteLog(3,0,$DT->{shopname}."が".$DTS->{shopname}."へ万引きに入り$ITEM[@@ITEMNO"店番ロボット"]->{name}を破壊しましたが結局捕まりました");
					}
					else
					{
						WriteLog(3,0,$DT->{shopname}."が".$DTS->{shopname}."へ万引きに入りましたが$ITEM[@@ITEMNO"店番ロボット"]->{name}に捕まりました");
					}
				}
				else
				{
					...
					$ret="万引きは成功しました";
					WriteLog(2,0,$DTS->{shopname}."が総額\\$manbiki_countの万引き被害に遭いました") if $manbiki_count;
					WriteLog(2,0,$DT->{shopname}.'からの犯行声明:「'.$USE->{arg}{message}.'」') if $USE->{arg}{message} && $manbiki_count;
	...
	@@use
		...
		func	_local_
			...
			WriteLog(2,0,$DT->{shopname}.'からの犯行声明:「'.$USE->{arg}{message}.'」') if $USE->{arg}{message};
			return $main::ITEMTYPE[$itemtype].'市場の悪い噂が広まったようです';
		_local_
@@event
	start		-1 #イベント自然発動無し
	code		kill-ken
...
#祝福イベント 追加:ver.2001-11-11f
@@event
	start		20%
	basetime	5h
	plustime	5h
	code		happy
	startmsg	祝福の光が辺りを照らしはじめました
	endmsg		祝福の光が消えました
	info		祝福の光がふりそそがれています(人気UP)
	func		_local_
		my $time=$TIMESPAN;
...
@@EVENT
	...
	startfunc	_local_(400,200)
		...
		foreach my $DT (@DT)
		{
			...
			return (0,$DT->{shopname}.'が総額\\'.$count.'の万引き被害に遭いました') if $count;
...
@@EVENT
	...
	startfunc	_local_(700)
		...
		foreach my $DT (@DT)
		{
			...
			return (0,$DT->{shopname}.'が総額\\'.$count.'の強盗被害に遭いました') if $count;
...
@@EVENT
	...
	startfunc	_local_(100000)
		...
		foreach(reverse(@DT))
		{
			...
			return (0,$_->{shopname}.'へ\\'.$money.'の補助金が支給されました');
		}
		return 0;
	_local_
		...
		foreach my $DT (@DT)
		{
			...
			foreach(1..$count)
			{
				...
				if($hit)
				{
					...
					WriteLog(($hit<=3?1:2),0,$DT->{shopname}."が$hit等\\$getmoneyを当てました！");
				}
			}
		}
		return 0;
...
{
	WriteLog(0,$DT->{id},0,'移転元の街から餞別として\\'.GetUserDataEx($DT,'_so_present_money').'をもらいました',1);
