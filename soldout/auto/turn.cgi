# $Id: turn.cgi 96 2004-03-12 12:25:28Z mu $
...
sub TurnProc
{
	...
	foreach $TM (@timespan)
	{
		...
		my $peopleup=0;
		my $peopledown=0;
		my $peopleupsalerate=($DTpeople/100000)**1.5;
		foreach my $DT (@DT)
		{
			$nowranking++;
			...
			foreach $cnt (0..$DT->{showcasecount}-1)
			{
				...
				if($itemno && $DT->{item}[$itemno-1])
				{
					...
					$rank=$DT->{temprank};
					$rank=99 if $rank<100;
					#####if($rank<5000){$rank=$rank/6000*$rank;}
					...
					#$disp.="$itemno:$DT->{id}:$buypeopletemp:$ITEM[$itemno]->{buypeoplebase}<BR>";
					
					#販売価格による人気UP/DOWN (廃止)
					#my $rankplus=rand((abs($rank>2 ? 1:$rank-1))*$timespan/120);
					#if($rank<1)
					#{
					#	#$rankdown+=$rankplus;
					#}
					#else
					#{
					#	#$rankplus*=2 if $DT->{rank}<5000; # 人気が50%未満なら人気上昇率2倍
					#	#$rankup+=$rankplus;
					#}
					...
					######$rank **= (int((1-$rank)*15+1)) if $rank<1;
					...
					if($sale>0)
					{
						...
						#$rankdown+=rand((0.5/$rank)*$sale);
						#$rankup  +=rand(($rank>2 ? 2*$rankupmulti : $rank*$rankupmulti)
						#                *$ITEM[$itemno]->{point}/1000*$sale);
						
						
						#売上処理
						my $saleprice=$sale*$DT->{price}[$cnt];
						
						my($taxrate,$tax)=GetSaleTax($itemno,$sale,$saleprice,GetUserTaxRate($DT));
						
						$DT->{moneystock}+=$saleprice-$tax;
						...
						#人口増加
						my $pu1=$DT->{price}[$cnt];
						my $pu2=$ITEM[$itemno]->{price};
						$peopleup+=$sale*($pu1<$pu2?$pu1:$pu2)/$peopleupsalerate+$sale;
				...
				{
					#売り切れの場合の人気DOWN
					#$rankdown+=$timespan/400 if $itemno; #とりあえずコメントアウト
				}
			}
			$salesumall+=$salesum;
			
			#時間経過による無条件人気DOWN
			# old pattern $rankdown+=($timespan)*(($DT->{rank}/1000)**$POP_DOWN_RATE)/(5**$POP_DOWN_RATE*400)*$DT->{showcasecount};
			$rankdown+=sqrt($timespan)*(($DT->{rank}/1000)**$POP_DOWN_RATE)/(5**$POP_DOWN_RATE*20)*$DT->{showcasecount};
			
			#時間経過による無条件人口DOWN
			$peopledown+=rand($DTpeople/10000*$timespan/3600);
		...
		#$peopledownと$peopleupによる人口UP/DOWN処理
		my $peopleupdown=abs($peopleup-$peopledown);
		if($peopleupdown<1 && $peopleupdown>0)
			{if(rand(1/$peopleupdown)<1){$peopleupdown=1;}}
		$DTpeople+=int($peopleup>$peopledown ? $peopleupdown:-$peopleupdown);
		if($DTpeople<100000){$DTpeople+=int(rand(100000-$DTpeople));}
		...
		#決算処理
		TurnPeriod() if $dateupdate && scalar(@DT);
			...
			foreach my $key (keys(%DTevent))
			{
				...
				if($DTevent{$key}<$temptimenow+$timespan)
				{
					...
					if($E->{endfunc})
					{
						...
						WriteLog(2,0,0,$msg,1) if $msg;
						undef &$func;
					}
					if($ret)
					{
						delete $DTevent{$key};
						WriteLog(2,0,0,$E->{endmsg},1) if $E->{endmsg};
					}
					else
					{
						$group{$E->{group}}=1;
		...
		foreach my $key (keys(%EVENT))
		{
			...
			if(rand(100000)<$timespan*$E->{startproba}/864 || $E->{startproba}==0)
			{
				...
				if($ret)
				{
					WriteLog(2,0,0,$E->{startmsg},1) if $E->{startmsg};
	...
	foreach my $DT (@DT)
	{
		if($DT->{lastlogin}+$EXPIRE_TIME+GetExpireTimeExtend($DT)<$NOW_TIME)
		{
			CloseShop($DT->{id},'経営者不在');
			WriteLog(1,0,0,$DT->{shopname}."が経営者不在により閉店しました",1);
		}
	}
